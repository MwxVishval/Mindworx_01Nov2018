...
// Start ============ get this function data for showing report flag    ------------------------------    
    public function getReportFieldFlagForEnity($ent_id) {
                ...
                $SqlReportdata = $this->db->select(QM_TASK_TYPE.'.id,'.QM_TASK_TYPE.'.report_flag,'.QM_TASK_REPORT.'.id,'.QM_TASK_REPORT.'.task_id,'.QM_TASK_REPORT.'.task_type_id,'.QM_TASK_REPORT.'.ent_id')
                                  ->from($table) 
...
// End ============ get this function data for showing Attachments    ------------------------------     
    ...
    public function insert_or_update_reportFields($field_name, $ent_id, $task_type, $updatedstatus,$flagstatus) {
        $table = QM_TASK_REPORT;   
...
//        exit;
        ...
        if ($query->num_rows()>0) { 
           ...
           $UpdatereportData = array(
            ...
            return $this->UpdateData($UpdatereportData, $id, $table);
